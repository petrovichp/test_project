...
> - **Capacity test — POSITIVE result** ([docs/capacity_test.md](docs/capacity_test.md)). Trained at hidden ∈ {64, 128, 256}. **VOTE5_h256 achieves WF mean Sharpe +11.86** (highest in project), single-seed mean WF +9.85 (vs h=64 +7.74, Δ +2.11). But fold-6 weakens with capacity (h=64 +5.20 → h=256 +0.41). VOTE5_h128 wins fold 6 (+10.70) and DQN-test (+10.59). **Three new baselines** with distinct profiles: BASELINE_VOTE5 (h=64, balanced), VOTE5_H128 (late-period winner), VOTE5_H256 (aggregate winner). Signal is **NOT capacity-saturated**.
