...
> - **P1 maker-on-TP fee model — NEGATIVE** ([docs/experiments/mixed_fee_eval.md](docs/experiments/mixed_fee_eval.md)). Treating TP exits as maker (2bp) and others as taker (4.5bp) lifts DISTILL_v8 by only +0.04 and VOTE5_v8 by +0.04 — bounded by 3% TP-hit rate. Strategies are designed with wide TPs + trailing stops; TIME stop (40%) and TSL (26%) dominate exits. Free to ship in production (+0.04 Sharpe is real money long-term) but not transformative.
> - **v10 regime ablation — NUANCED** ([docs/experiments/v10_regime_ablation.md](docs/experiments/v10_regime_ablation.md)). Removing regime dims [2:7] improves single-seed policies (test +6.69 vs +4.39 v8 family-mean) but hurts the K=5 ensemble (val collapses +6.67 → +0.85). Bias-variance: regime is per-seed shortcut but injects ensemble diversity. DISTILL_v10 worse than DISTILL_v8 (student inherits weakened plurality labels). Freeze unchanged.
> - **A1 + A2 + A3 audits** ([docs/audits/](docs/audits/)). State-pack layout verified (1 docstring drift in transformer_network.py). Regime classifier verdict revised from "weak/non-stationary" to "informative but largely redundant" (regime predictable from vol_pred/atr/bb_width with F-stats 10k-22k; P(positive|regime) is stable across splits — mean sign-flip was outlier artifact). Signal recency: DISTILL alive (fold-6/mean=85%), VOTE5 borderline-but-environment (37%; recent calm-uptrend is structurally hard for the strategies).
>
> **🔒 Status (2026-05-13, post audit-sweep A1/A2/A3 + v10 regime ablation):**
> - **A1 — State-pack consistency audit** ([docs/audits/state_pack_v8_layout.md](docs/audits/state_pack_v8_layout.md)). Definitive 52-dim layout from `dqn_state.py` builder. Found `transformer_network.py` docstring had wrong dim assignments (direction probs at [0:3], hour-of-day at [16:17], etc.) — Z4.2's negative result is even less surprising given the actual state is mostly windowed lag-features, not 52 semantic tokens. Also: S12_VWAPVol (dim 15) only fires {-1, 0} on val/test — effectively short-only in deployment.
> - **A2 — Regime classifier (CUSUM+Hurst v4) — INFORMATIVE BUT LARGELY REDUNDANT** ([docs/audits/regime_classifier_quality.md](docs/audits/regime_classifier_quality.md)). Original verdict "weak / non-stationary" was overstated. Deeper re-analysis: (1) P(positive return | regime) is stable across splits (the mean sign-flip was a fat-tailed outlier artifact); (2) F-stats 10k-22k show regime is a non-linear quantization of `vol_pred / atr_pred_norm / bb_width` already in state — largely shortcut, not independent signal; (3) classifier names are correct at 24h+ horizons (just not 60m).
> - **A3 — Signal recency on fold-6 — ALIVE for DISTILL, BORDERLINE for VOTE5** ([docs/audits/signal_recency.md](docs/audits/signal_recency.md)). DISTILL_v8_seed42 fold-6/mean = 85% (test +9.35 > fold-6 +8.50 > all-fold mean +9.99). VOTE5_v8 fold-6/mean = 37% but reflects environment (recent fold = calm BTC uptrend; VOTE5's biggest Sharpes come from volatile-decline folds). Decision: P1 maker-fill feasibility GO for tomorrow; VOTE5 weakness is environment, not decay.
> - **v10 regime ablation — NUANCED** ([docs/experiments/v10_regime_ablation.md](docs/experiments/v10_regime_ablation.md)). Retrained `VOTE5_v10_H256_DD` (5 seeds) with state[:, 2:7] zeroed. **At single-seed level: v10 is BETTER** (WF +9.83 vs +8.33 v8 single-seed family-mean, test +6.69 vs +4.39). **At ensemble level: v10 is WORSE** (WF +10.75 vs +12.07 v8; val collapses to +0.85 vs +6.67; 5/6 folds). Mechanism is bias-variance: regime is per-seed shortcut but injects ensemble diversity. Distillation `DISTILL_v10_seed42` is worse than DISTILL_v8 across all metrics (test +6.14 vs +9.35) — student inherits weakened plurality labels. **Freeze decision unchanged**: dual deployment of DISTILL_v8 + VOTE5_v8 remains optimal.
>
> **🔒 Status (2026-05-12, post Z4 + Z5 + audit corrections — FROZEN PRODUCTION BASELINES):**
> - **Z5 audit correction (same-day)** ([docs/experiments/z5_validation.md](docs/experiments/z5_validation.md) §Audit). The original Z5.1 regime_shuffle perturbed an unused sidecar array instead of `state[:, 2:7]` (the actual model input). Two prior Z5.1 findings withdrawn: **"regime not in state vector"** is wrong (regime IS at state[:, 2:7]); **"long-bias asymmetry"** is unsupported (the inversion test was confounded). Corrected diagnostics ([models/audit_regime_direction.py](models/audit_regime_direction.py)) show: **(Q1)** DISTILL_v8 degrades −2 to −6 Sharpe under regime perturbation — regime info IS used; **(Q2)** both policies trade ~55/45 long/short with shorts contributing MORE PnL — no long-bias in deployment; **(Q3)** all 5 regimes profitable, trend_down is the highest per-trade-PnL bucket. Freeze decision unchanged (rested on Z5.2/Z5.3, not Z5.1).
> - **Z4.4 QR-DQN with CVaR — NEGATIVE-LEAN** ([docs/experiments/z4_qrdqn_transformer.md](docs/experiments/z4_qrdqn_transformer.md)). Trained K=5 QRDuelingDQN (32 quantiles) with CVaR-α action selection. CVaR=0.3 is the best alpha (vs 0.1/0.5/1.0). Ensemble WF +11.58, val +4.88, test +5.12 — **does NOT beat VOTE5_v8** (WF +12.07) or **DISTILL_v8_seed42** (val +10.41, test +9.35). Best single seed (s=0) hits test +7.95.
> - **Z4.2 Transformer-DQN — NEGATIVE** ([docs/experiments/z4_qrdqn_transformer.md](docs/experiments/z4_qrdqn_transformer.md)). Lighter `TransformerDQN(d_model=8, n_heads=2, n_layers=1, hidden=128)`, 5 seeds K=5 plurality: **WF +6.35**, val +4.88, test +5.95 — well below both baselines. Seed 0 outlier sets project-record single-seed val (+11.11) but voting dilutes it. Distillation of the XFMR ensemble (`DISTILL_XFMR_v8`): val +9.41, **test +2.83** — student inherits weak teacher. Multi-head attention's inductive bias doesn't match this signal.
> - **Z5.1 OOD stress (revised)** ([docs/experiments/z5_validation.md](docs/experiments/z5_validation.md)). Price inversion result (−6 to −10 Sharpe) reflects feature-price correlation, NOT long-bias — **withdrawn as evidence of asymmetry**. Regime shuffle (original): **buggy, withdrawn**. Feature noise σ=0.1 on dims [30:50]: DISTILL robust on val (+0.17 Δ), teacher fragile (−4.18 Δ) — **stands**.
