...
> - **Direction probabilities in state — NEGATIVE result** ([docs/state_v6_test.md](docs/state_v6_test.md)). Tested adding 4 CNN-LSTM direction-prediction outputs to the state (50→54 dims). VOTE5_v6 ensemble: WF Sharpe **−1.78** (8.63 vs 10.40), val −2.31, fold 6 −1.31. The probs are already implicit in the binary signal flags (signals are computed FROM these probs); raw exposure adds redundant noise. State distribution shift across train/val/test compounds the issue. **Bottleneck is not in input information — re-orient to network capacity, reward shaping, or different RL algorithms.**
